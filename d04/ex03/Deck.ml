...
let newDeck () =
    Random.self_init ();
    let rec getCard deck =
        if List.length deck = 52 then
            deck
        else
            let rec findCard card =
                if List.mem card deck = false then
                    card
                else
                    findCard (List.nth Card.all (Random.int 52))
            in getCard ((findCard (List.nth Card.all (Random.int 52))) :: deck)
    in getCard []
...
let drawCard t =
	if t = [] then
		failwith "Your deck is empty, you'r dead bro !"
	else
	    (List.hd t, List.tl t)
